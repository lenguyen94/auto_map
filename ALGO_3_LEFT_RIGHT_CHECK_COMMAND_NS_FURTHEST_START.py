...
rotation_needed = 0
rotation_amount = 0 #angle in radian
...
def Walker (Data):
    ...
    global prev_euler_angle, prev_nearest_object_angle, rotation_needed, rotation_amount, rotation_direction, prev_furthest_object_angle, start_now
    ...
    if command == "move":
	if start_now == 1:
			#TB3 will rotate to the furthest distance it can move at the start
            prev_furthest_object_angle = furthest_object_angle
            TakeFurthestDistance()
...
def TurnCheck (predict_range, predict_orientation, count):
    ...
    predict_TB3_VIEW_MIN = new_predict_orientation_centre - 23

    predict_TB3_VIEW_MAX = new_predict_orientation_centre + 22

    angle = 0
    predict_nearest_object_dist = 0
    predict_nearest_object_angle = 0

    for x in np.nditer(numpy_predict_range, flags=['zerosize_ok']): #zerosize_ok flag is to allow array size of 0
        if angle >= predict_TB3_VIEW_MIN and angle <= predict_TB3_VIEW_MAX:
            if (angle == predict_TB3_VIEW_MIN): #for initial angle value, reinitialize nearest object distance and angle
                if x == 0:
                ...
                if x < predict_nearest_object_dist and x != 0: #the smaller the value, the closer the object is
                    predict_nearest_object_dist = x
                    predict_nearest_object_angle = angle
        ...
        if angle > predict_TB3_VIEW_MAX:
        	...
        	##		2. Turning LEFT-LEFT or RIGHT-RIGHT
        	##

            if (predict_nearest_object_dist < DISTANCE_LIMIT and count < 3):
		# LEFT-RIGHT or RIGHT-LEFT. Not ok
		    ...
		    count = count + 1
            	    if (count == 3): #just take furthest distance direction
		    	TakeFurthestDistance()
...
def TakeFurthestDistance ():
	
	global rotation_direction, rotation_amount, prev_furthest_object_angle

	if prev_furthest_object_angle <= 179: #179 is 180 degrees
		rotation_direction = "RIGHT"
		rotation_amount = (((179 - prev_furthest_object_angle) * PI) / 180.0)

 	elif prev_furthest_object_angle > 179:
		rotation_direction = "LEFT"
		rotation_amount = (((prev_furthest_object_angle - 179) * PI) / 180.0)
